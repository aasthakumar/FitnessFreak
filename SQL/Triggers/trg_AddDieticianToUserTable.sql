...
DROP TRIGGER IF EXISTS AddDieticianToUserTable;

DELIMITER $$

CREATE TRIGGER AddDieticianToUserTable
    AFTER INSERT ON Dietician
    FOR EACH ROW 
BEGIN
        ...
        INTO f_name,m_name,l_name,email_addr FROM Dietician 
        WHERE DieticianID = new.DieticianID;
		...
		VALUES
						(f_name,m_name, l_name, MD5('dietician'), 0, NOW(), 0, 0, DATE_ADD(NOW(), INTERVAL 1 YEAR) ,'9999-12-12',
						 'N/A', 'N/A' ,'N/A' ,'N/A' ,'12345' ,email_addr ,0, 'SILVER','Basic Subscription',0,0,'DIETICIAN');
